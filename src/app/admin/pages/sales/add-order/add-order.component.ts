...
export class AddOrderComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    ...
    const orderData: any = {
      ...
      thana: this.dataForm.value.thana,
  ...
  get grandTotal(): number {
    return ((this.cartSubTotal + this.deliveryCharge)-this.discount)
  }
        ...
        if (dialogResult) {
          if (this.id) {
            ...
            if (this.dataForm.value.hasOrderTimeline) {
              ...
              else if(this.dataForm.value.orderStatus === OrderStatus.ReadToPrint) {
                data.orderTimeline.ReadToPrint = {
                  success: true,
                  date: new Date(),
                  expectedDate: null,
                }
              }
              else if(this.dataForm.value.orderStatus === OrderStatus.ReadyToPackaging) {
                data.orderTimeline.ReadyToPackaging = {
                  success: true,
                  date: new Date(),
                  expectedDate: null,
                }
              }
              else if(this.dataForm.value.Packaging === OrderStatus.Packaging) {
                data.orderTimeline.Packaging = {
                  success: true,
                  date: new Date(),
                  expectedDate: null,
                }
              }
              else if(this.dataForm.value.ReadyToShipped === OrderStatus.ReadyToShipped) {
                data.orderTimeline.ReadyToShipped = {
                  success: true,
                  date: new Date(),
                  expectedDate: null,
                }
              }
              else if(this.dataForm.value.HoldInvoice === OrderStatus.HoldInvoice) {
                data.orderTimeline.HoldInvoice = {
                  success: true,
                  date: new Date(),
                  expectedDate: null,
                }
            ...
            if (this.dataForm.value.hasOrderTimeline) {
              data.orderTimeline = {
                ...
                ReadToPrint: {
                  success: this.dataForm.value.orderStatus === OrderStatus.ReadToPrint,
                  date: this.dataForm.value.orderStatus === OrderStatus.ReadToPrint ? new Date() : null,
                  expectedDate: null,
                },
                ReadyToPackaging: {
                  success: this.dataForm.value.orderStatus === OrderStatus.ReadyToPackaging,
                  date: this.dataForm.value.orderStatus === OrderStatus.ReadyToPackaging ? new Date() : null,
                  expectedDate: null,
                },
                Packaging: {
                  success: this.dataForm.value.orderStatus === OrderStatus.Packaging,
                  date: this.dataForm.value.orderStatus === OrderStatus.Packaging ? new Date() : null,
                  expectedDate: null,
                },
                ReadyToShipped: {
                  success: this.dataForm.value.orderStatus === OrderStatus.ReadyToShipped,
                  date: this.dataForm.value.orderStatus === OrderStatus.ReadyToShipped ? new Date() : null,
                  expectedDate: null,
                },
                HoldInvoice: {
                  success: this.dataForm.value.orderStatus === OrderStatus.HoldInvoice,
                  date: this.dataForm.value.orderStatus === OrderStatus.HoldInvoice ? new Date() : null,
                  expectedDate: null,
                },
              };
            }
