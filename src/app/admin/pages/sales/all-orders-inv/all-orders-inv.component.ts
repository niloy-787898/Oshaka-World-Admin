...
@Component({
  selector: 'app-all-orders-inv',
  templateUrl: './all-orders-inv.component.html',
  styleUrls: ['./all-orders-inv.component.scss'],
})
export class AllOrdersInvComponent implements OnInit {
  // Admin Base Data
  adminId: string;
  role: string;
  ...
  private deleteOrderById(id: string) {
    this.spinner.show();
    this.subDataFive = this.orderService.deleteOrderById(id).subscribe(
      (res) => {
        this.spinner.hide();
        if (res.success) {
